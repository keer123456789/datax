...
public class CommonRdbmsReader {
    ...
    public static class Task {
        ...
        public void startRead(Configuration readerSliceConfig,
            ...
            if (this.dataBaseType.equals(DataBaseType.HIVE) && readerSliceConfig.getBool(Key.HAVE_KERBEROS, false)) { //hive数据库 且 需要kerberos认证
                System.setProperty("java.security.krb5.conf", readerSliceConfig.getString(Key.KERBEROS_CONFIG_FILE_PATH));
                org.apache.hadoop.conf.Configuration conf = new org.apache.hadoop.conf.Configuration();
                conf.addResource(readerSliceConfig.getString(Key.KERBEROS_HDFSCONF_FILE_PATH));
                conf.addResource(readerSliceConfig.getString(Key.KERBEROS_CORECONF_FILE_PATH));
                conf.set("hadoop.security.authentication", "Kerberos");
                UserGroupInformation.setConfiguration(conf);
                try {
                    UserGroupInformation.loginUserFromKeytab(readerSliceConfig.getString(Key.KERBEROS_USERNAME), readerSliceConfig.getString(Key.KERBEROS_KEYTAB_FILE_PATH));
                } catch (Exception e) {
                    LOG.error("KERBEROS 认证失败，请检查kerberos 相关配置是否正确，错误信息：{}", e.getMessage());
                }
            }
            int columnNumber = 0;
            ResultSet rs = null;
