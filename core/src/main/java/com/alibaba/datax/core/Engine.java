...
public class Engine {
    ...
    public Map mockStart(Configuration allConf) {
        ...
        PerfTrace perfTrace = PerfTrace.getInstance(isJob, instanceId, taskGroupId, priority, traceEnable);
        perfTrace.setJobInfo(jobInfoConfig, perfReportEnable, channelNumber);
        return container.start(new HashMap());
    }
    ...
    public static Map mockEntry(final String[] args) throws Throwable {
        Options options = new Options();
        options.addOption("job", true, "Job config.");
        options.addOption("jobid", true, "Job unique id.");
        options.addOption("mode", true, "Job runtime mode.");
        ...
        ConfigurationValidate.doValidate(configuration);
        Engine engine = new Engine();
        return engine.mockStart(configuration);
    }
    ...
    public static void main(String[] args) throws Exception {
        ...
        try {
            ...
            Map result = Engine.mockEntry(args);
            String basePath = System.getProperty("user.dir");
            RuntimeMXBean runtimeMXBean = ManagementFactory.getRuntimeMXBean();
            int processId = Integer.valueOf(runtimeMXBean.getName().split("@")[0]).intValue();
            String filePath = basePath + "/result/" + processId + ".json";
            File file = new File(filePath);
            if (!file.exists()) {
                file.createNewFile();
            }
            PrintStream ps = new PrintStream(new FileOutputStream(file));
            ps.println(JSON.toJSONString(result));// 往文件里写入字符串
            ps.close();
        } catch (Throwable e) {
            exitCode = 1;
